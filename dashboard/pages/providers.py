...
def render():
    ...
    st.warning("⚠️ Paid providers are for manual turbo mode only. Scheduled workers always use local models.")
    ...
    for profile_name, profile in profiles.items():
        ...
        with st.container():
            ...
            if not has_key:
                ...
                with cols[0]:
                    if st.button(f"🗑️ Remove", key=f"remove_{profile_name}"):
                        if remove_env_key(env_var):
                            st.success(f"✅ {provider} key removed!")
                            st.rerun()

                with cols[1]:
                    if st.button(f"🧪 Test", key=f"test_{profile_name}"):
                        code, out = run_ps(
